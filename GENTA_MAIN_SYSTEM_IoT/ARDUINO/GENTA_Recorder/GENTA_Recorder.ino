...
// Note: switched WS/SCK to match wiring: WS -> GPIO33, SCK -> GPIO32
#define I2S_WS 25
#define I2S_SD 26
#define I2S_SCK 33
...
// Force I2S channel: 0=auto, 1=force LEFT, 2=force RIGHT (can be set via /diag?force=left|right|auto)
volatile int forceI2SChannel = 0;

// ---------- OLED DISPLAY ----------
#define I2C_SDA 21
#define I2C_SCL 22   // Match wiring: SDA on GPIO21, SCL on GPIO22 (as per top comment)
...
static void sad_eyes() {
  ...
  // More expressive sad eyes: eyes droop, inner corners lift slightly,
  // and a small tear appears under the right eye. We interpolate
  // from the idle pose to the final droopy pose for a smooth effect.
  reset_eyes(true);
  ...
  for (int f = 0; f < frames; ++f) {
    // t ranges 0..1
    float t = (frames <= 1) ? 1.0f : ((float)f / (float)(frames - 1));

    // vertical droop (downwards)
    int droopY = (int)(t * 8.0f); // up to 8px down
    left_eye.y = startL.y + droopY;
    right_eye.y = startR.y + droopY;

    // reduce height to create droopy eyelid appearance
    int dropH = (int)(t * 10.0f); // reduce up to 10px
    left_eye.height = max(4, startL.height - dropH);
    right_eye.height = max(4, startR.height - dropH);

    // slightly widen horizontally so droop looks natural
    left_eye.width = startL.width + (int)(t * 4.0f);
    right_eye.width = startR.width + (int)(t * 4.0f);

    // inner-corner lift: eyes move subtly inward to emphasize sadness
    int inward = (int)(t * 4.0f);
    left_eye.x = startL.x - inward;
    right_eye.x = startR.x + inward;

    // soften corner radius as lids droop
    int newCorner = REF_CORNER_RADIUS - (int)(t * 6.0f);
    if (newCorner < 1) newCorner = 1;
    corner_radius = newCorner;

    draw_frame();
    delay(60);
  }

  // Draw a small tear that falls from the right eye
  int tearX = right_eye.x + (right_eye.width / 4);
  int tearY = right_eye.y + (right_eye.height / 2) + 6;
  for (int k = 0; k < 3; ++k) {
    // redraw eyes then overlay a small tear disc so it blends naturally
    draw_frame();
    u8g2.setDrawColor(1);
    u8g2.drawDisc(tearX, tearY + k * 2, 1);
    u8g2.sendBuffer();
    delay(160);
  }
...
// WiFi connected animation: small WiFi arcs + checkmark, then return to idle
static void wifi_connected_animation() {
  if (!oledReady) return;
  int cx = SCREEN_WIDTH / 2;
  int cy = SCREEN_HEIGHT / 2;

  // Brief 6-frame animation: reveal arcs then checkmark
  // We will draw semi-arcs by drawing circles then masking the lower half
  const int maxR = 20;
  for (int frame = 0; frame < 6; ++frame) {
    u8g2.clearBuffer();
    // Title
    u8g2.setFont(u8g2_font_6x10_tf);
    const char* lbl = "WiFi Connected";
    int tw = u8g2.getStrWidth(lbl);
    u8g2.drawStr((SCREEN_WIDTH - tw) / 2, 12, lbl);

    // Draw concentric circles to represent arcs (largest first)
    for (int i = 0; i < 3; ++i) {
      int needFrame = i + 1; // arc i appears on frame >= needFrame
      if (frame >= needFrame) {
        int r = maxR - i * 6;
        u8g2.drawCircle(cx, cy + 6, r);
      }
    }

    // Mask the lower half to leave only arcs (semi-circles)
    // Set draw color to background (0) to erase lower portion
    u8g2.setDrawColor(0);
    u8g2.drawBox(0, cy + 6, SCREEN_WIDTH, SCREEN_HEIGHT - (cy + 6));
    // Restore draw color to white
    u8g2.setDrawColor(1);

    // On later frames draw a checkmark overlay
    if (frame >= 3) {
      // simple two-segment check mark
      u8g2.drawLine(cx - 8, cy + 2, cx - 2, cy + 8);
      u8g2.drawLine(cx - 2, cy + 8, cx + 10, cy - 6);
    }
  ...
  // Hold final frame briefly then show booting animation instead of eyes
  delay(700);
  // Switch to booting animation to indicate device is ready but awaiting
  // the user's explicit 'start' to wake up the main UI.
  booting_animation();
}
...
bool connectToWiFi(const String &ssid, const String &password, int timeout_seconds = 30) {
  ...
  WiFi.mode(WIFI_STA);
  // Small pause to let the network stack settle before further init to avoid
  // spurious "netstack cb reg failed" race conditions on some cores/coresets.
  delay(200);
  delay(100);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Defer the WiFi-connected animation until higher-level services
    // (UDP announcements, mDNS and the HTTP server) are initialized.
    // The animation will be triggered from setup() after those steps.
    return true;
  } else {
...
void startRecording(){
  ...
  // Prepare pin config for I2S and perform an automatic channel detection.
  ...
  // Try to detect whether the INMP441 is outputting on LEFT or RIGHT channel.
  // This installs a temporary stereo I2S driver, reads a short buffer and
  // compares the activity on each channel. Returns 1=LEFT, 2=RIGHT, 0=unknown.
  auto detect_i2s_channel = [&pin_config]() -> int {
    Serial.println("I2S: Starting channel autodetect (stereo read)");
    i2s_config_t detect_cfg = {
    ...
    esp_err_t er = i2s_driver_install(I2S_PORT, &detect_cfg, 0, NULL);
    if (er != ESP_OK) {
      Serial.printf("I2S detect: driver install failed 0x%08X\n", er);
      return 0;
    }
    i2s_set_pin(I2S_PORT, &pin_config);
    i2s_zero_dma_buffer(I2S_PORT);

    const size_t bufBytes = 8192; // longer capture for stable statistics
    uint8_t *buf = (uint8_t*) heap_caps_malloc(bufBytes, MALLOC_CAP_DMA);
    if (!buf) {
      Serial.println("I2S detect: failed to alloc buffer");
      i2s_driver_uninstall(I2S_PORT);
      return 0;
    }

    size_t bytesRead = 0;
    // Read a burst (timeout 700ms)
    esp_err_t r = i2s_read(I2S_PORT, buf, bufBytes, &bytesRead, pdMS_TO_TICKS(700));
    if (r != ESP_OK || bytesRead < 4) {
      Serial.printf("I2S detect: read failed 0x%08X bytes=%u\n", r, (unsigned)bytesRead);
      heap_caps_free(buf);
      i2s_driver_uninstall(I2S_PORT);
      return 0;
    }

    // Analyze stereo interleaved 16-bit samples: [L][R][L][R] ... each sample is 2 bytes
    size_t frames = bytesRead / 4; // each frame = 4 bytes (2 bytes L + 2 bytes R)
    uint64_t leftSumAbs = 0;
    uint64_t rightSumAbs = 0;
    for (size_t f = 0; f < frames; ++f) {
      int idx = f * 4;
      int16_t l = (int16_t)((uint16_t)buf[idx] | ((uint16_t)buf[idx+1] << 8));
      int16_t rgt = (int16_t)((uint16_t)buf[idx+2] | ((uint16_t)buf[idx+3] << 8));
      leftSumAbs += (uint32_t)abs((int)l);
      rightSumAbs += (uint32_t)abs((int)rgt);
    }

    // Compute mean absolute values (usefloat for ratio calc)
    float leftMean = (frames > 0) ? ((float)leftSumAbs / (float)frames) : 0.0f;
    float rightMean = (frames > 0) ? ((float)rightSumAbs / (float)frames) : 0.0f;

    heap_caps_free(buf);
    i2s_driver_uninstall(I2S_PORT);

    Serial.printf("I2S detect: frames=%u leftMean=%.2f rightMean=%.2f\n", (unsigned)frames, leftMean, rightMean);
    if (leftMean <= 0.001f && rightMean <= 0.001f) return 0;

    // Require a 20% margin to select a channel; otherwise treat as ambiguous
    const float RATIO_TOL = 1.20f;
    if (leftMean > rightMean * RATIO_TOL) return 1;  // left dominant
    if (rightMean > leftMean * RATIO_TOL) return 2; // right dominant
    // Ambiguous: return 0 to indicate undecided
    return 0;
  };

  int detected = detect_i2s_channel();
  // Allow runtime override
  if (forceI2SChannel == 1) {
    Serial.println("I2S: forcing LEFT channel via forceI2SChannel");
    detected = 1;
  } else if (forceI2SChannel == 2) {
    Serial.println("I2S: forcing RIGHT channel via forceI2SChannel");
    detected = 2;
  }

  if (detected == 2) Serial.println("I2S autodetect: microphone appears on RIGHT channel -> configuring ONLY_RIGHT");
  else if (detected == 1) Serial.println("I2S autodetect: microphone appears on LEFT channel -> configuring ONLY_LEFT");
  else Serial.println("I2S autodetect: unable to determine channel; defaulting to LEFT (ambiguous)");

  i2s_config_t i2s_config = {
    .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX),
    .sample_rate = I2S_SAMPLE_RATE,
    .bits_per_sample = (i2s_bits_per_sample_t)I2S_BITS_PER_SAMPLE_16BIT,
    .channel_format = (detected == 2) ? I2S_CHANNEL_FMT_ONLY_RIGHT : I2S_CHANNEL_FMT_ONLY_LEFT,
    .communication_format = (i2s_comm_format_t)(I2S_COMM_FORMAT_I2S | I2S_COMM_FORMAT_I2S_MSB),
    .intr_alloc_flags = ESP_INTR_FLAG_LEVEL1,
...
void setup(){
  ...
  oledBegin();
  // If OLED is available, show an immediate boot animation so power-on displays
  // a loading indicator right away (helps give immediate feedback on power-up).
  if (oledReady) {
    oledShowMode(OLED_BOOTING);
    // Play a short burst of the boot animation frames immediately on setup
    for (int __b = 0; __b < 12; __b++) { booting_animation(); delay(75); }
  }
  if (!oledReady) {
  ...
  // Start mDNS only if we actually connected to a network; give the stack a
  // short moment to stabilize first. This reduces transient registration errs.
  if (wifiConnected) {
    delay(200);
  ...
  // Diagnostic endpoint: shows I2C scan and allows forcing I2S channel
  // Usage: /diag            -> returns JSON with I2C addresses and current force state
  //        /diag?force=left  -> force LEFT channel (forceI2SChannel=1)
  //        /diag?force=right -> force RIGHT channel (forceI2SChannel=2)
  //        /diag?force=auto  -> clear force (forceI2SChannel=0)
  server.on("/diag", HTTP_GET, [](){
    String forceAction = "";
    if (server.hasArg("force")) {
      forceAction = server.arg("force");
      forceAction.toLowerCase();
      if (forceAction == "left") { forceI2SChannel = 1; }
      else if (forceAction == "right") { forceI2SChannel = 2; }
      else if (forceAction == "auto") { forceI2SChannel = 0; }
    }

    // Perform a brief I2C scan
    String json = "{";
    json += "\"i2c\":[],"; // fill dynamically
    String addrs = "[";
    bool first = true;
    for (byte a = 1; a < 127; a++) {
      Wire.beginTransmission(a);
    ...
    json += addrs;
    json += ",\"force\":";
    if (forceI2SChannel == 1) json += "\"left\"";
    else if (forceI2SChannel == 2) json += "\"right\"";
    else json += "\"auto\"";
    json += ",\"forceValue\":" + String(forceI2SChannel);
    json += "}";
  ...
  // Clear stored WiFi credentials and restart (convenience endpoint)
  server.on("/wifi/clear", HTTP_GET, [](){
    Serial.println("HTTP: Clearing saved WiFi credentials (via /wifi/clear)");
  ...
  // Now that UDP announcements, mDNS and HTTP server are up, show the
  // WiFi-connected animation (deferred from connectToWiFi()). This
  // ensures the animation appears after the full network stack and
  // services are initialized (matching the expected startup logs).
  if (wifiConnected && oledReady) {
    wifi_connected_animation();
  }
}

void loop(){
  // Handle recording button (GPIO 22)
