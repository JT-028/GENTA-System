...
// State management (GPIO 23 button)
const int stateButtonPin = 23; // GPIO 23 for state toggle
...
// Rate-limit state changes to avoid rapid toggles from network or bounce
unsigned long lastStateChangeMillis = 0;
const unsigned long STATE_CHANGE_MIN_INTERVAL_MS = 1000; // 1 second
...
// SD support removed: device will use LittleFS (SPIFFS) for uploads/storage
...
// Play WAV file from SD card (same parser as SPIFFS version)
// SD playback removed; use LittleFS playback via playWavFromSPIFFS()
...
void setup() {
  ...
  // SD card support removed: use LittleFS only
  Serial.println("SD support disabled in firmware; using LittleFS only.");
  ...
  server.on("/upload", HTTP_POST, [](){
    ...
    if (upload.status == UPLOAD_FILE_START) {
      ...
      Serial.println("Upload: start (LittleFS)");
      ...
      if (upFile) {
        upFile.close();
        unsigned int sz = 0;
        File info = SPIFFS.open(tmpPath, FILE_READ);
  ...
  // /upload_sd removed: firmware no longer supports SD card storage. Use /upload instead.
  ...
  // Delete/remove an uploaded file from SD or SPIFFS
  // Supports query param: name=<basename or /path/to/file>
  // Routes: /delete, /remove, /delete_file, /remove_file, /rm
  auto _delete_handler = [](){
    ...
    // Protect critical files: do not allow deletion of certificate or state files
    // Add any other filenames here that must never be removed remotely.
    const char* protected_files[] = {"/ca.pem", "/state.txt", "/config.json"};
    for (size_t i = 0; i < sizeof(protected_files)/sizeof(protected_files[0]); ++i) {
      if (path.equalsIgnoreCase(protected_files[i])) {
        Serial.printf("/delete: attempt to remove protected file blocked: %s\n", path.c_str());
    ...
    bool removed = false;
    // Attempt removal from LittleFS only (SD support removed)
    if (SPIFFS.exists(path.c_str())) {
      if (SPIFFS.remove(path.c_str())) {
        removed = true;
        Serial.printf("/delete: removed from SPIFFS: %s\n", path.c_str());
      }
    }
  ...
  server.on("/delete", HTTP_ANY, _delete_handler);
  server.on("/remove", HTTP_ANY, _delete_handler);
  server.on("/delete_file", HTTP_ANY, _delete_handler);
  server.on("/remove_file", HTTP_ANY, _delete_handler);
  server.on("/rm", HTTP_ANY, _delete_handler);
      }
    }
  });
  ...
  server.on("/set_state", HTTP_GET, [](){
    if (server.hasArg("value")) {
      ...
      if (newState == "0" || newState == "1") {
        ...
        Serial.printf("/set_state request from %s -> %s\n", remote.toString().c_str(), newState.c_str());

        // Rate-limit remote state changes
        if (millis() - lastStateChangeMillis < STATE_CHANGE_MIN_INTERVAL_MS) {
          Serial.println("HTTP state change ignored due to rapid requests");
...
void loop() {
  // Handle state toggle button (GPIO 23)
  static bool prevStateBtn = HIGH;
  bool curStateBtn = digitalRead(stateButtonPin);
  if (prevStateBtn == HIGH && curStateBtn == LOW) {
    delay(150); // Debounce & require short hold to avoid spurious triggers
    if (digitalRead(stateButtonPin) == LOW) {
      Serial.println("*** STATE BUTTON PRESSED (GPIO 23) ***");
      ...
      if (!stateButtonEnabled) {
        ...
        if (millis() - lastStateChangeMillis < STATE_CHANGE_MIN_INTERVAL_MS) {
          Serial.println("State toggle ignored due to rapid presses");
        } else {
